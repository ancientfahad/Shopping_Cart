  ...
  addToCartButtons.forEach((button) => {
    button.addEventListener("click", () => {
      ...
      if (cart[productId]) {
        ...
        console.log(
          `Added to Cart - ID: ${product.id}, Title: ${product.title}, Quantity: ${cart[productId].quantity}`
        );
      } else {
        ...
        console.log(
          `Added to Cart - ID: ${product.id}, Title: ${product.title}`
        );
      }
    ...
    button.addEventListener("click", () => {
      ...
      if (cart[productId]) {
        if (cart[productId].quantity > 1) {
          ...
          cart[productId].quantity -= 1;
          console.log(
            `Removed one unit from Cart - ID: ${product.id}, Title: ${product.title}, Quantity: ${cart[productId].quantity}`
          );
        } else {
        ...
        console.log(
          `Product not in cart - ID: ${product.id}, Title: ${product.title}`
        ); // Log if not in cart
      }
    });
  });
...
function updateCartCount() {
  ...
  const cartCountNav = document.getElementById("cart-quantity");
